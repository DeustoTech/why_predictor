...
class TestShiftedRFRegressorBasic(unittest.TestCase):
    """Tests for ShiftedRFRegressor (Basic info)"""
    ...
    def setUp(self) -> None:
        ...
        self.model = rf.ShiftedRFRegressor(self.hyperparams, "tests/results")
...
class TestShiftedRFRegressorModel(unittest.TestCase):
    """Tests for ShiftedRFRegressor (Model generated)"""
    ...
    def test_calculate_errors_per_file(self) -> None:
        ...
        for col in errors.columns[2:]:
            ...
            for value in errors[col]:
                self.assertGreaterEqual(value, 0.0)
                self.assertLessEqual(value, 1.7)
        self.assertTrue(
            os.path.exists(
                "tests/results/errors/raw/"
                + f"{ErrorType.MAPE2.name}_{SHORT_NAME}_"
