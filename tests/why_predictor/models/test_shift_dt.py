"""Test for DecissionTree regression"""
import json
import os
import shutil
import unittest
...
from why_predictor.models import tree_regression as dt

NAME = "Shifted Decission Tree Regression"
SHORT_NAME = "SHIFT_DT"


class TestShiftedDecissionTreeRegressorBasic(unittest.TestCase):
    """Tests for ShiftedDecissionTreeRegressor (Basic info)"""
    ...
    def setUp(self):
        ...
        self.hyperparams = {"max_depth": 15}
        self.model = dt.ShiftedDecissionTreeRegressor(
...
class TestShiftedDecissionTreeRegressorModel(unittest.TestCase):
    """Tests for ShiftedDecissionTreeRegressor (Model generated)"""
    ...
    def test_make_predictions(self):
        ...
        for col in dtf.columns[2:]:
            self.assertIn(dtf[col].dtype, [np.uint16, np.uint8])
    ...
    def test_calculate_errors_per_file(self):
        ...
        for col in errors.columns[2:]:
            ...
            for value in errors[col]:
                self.assertGreaterEqual(value, 0.0)
                self.assertLessEqual(value, 6.5)
        self.assertTrue(
            os.path.exists(
                "tests/results/errors/raw/"
                + f"{ErrorType.MAPE2.name}_{SHORT_NAME}_"
    ...
    def test_calculate_timeseries_errors(self):
        ...
        # Validate
        self.assertGreaterEqual(value, 0.0)
        self.assertLessEqual(value, 150)
